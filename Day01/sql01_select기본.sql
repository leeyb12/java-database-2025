...
-- 03. 02번의 쿼리를 컬럼명을 변경해서 출력하시오. '직업번호', '이름', '성', '이메일', '입사일자'로 변경하시오(107행) 
-- alia 별명
SELECT employee_id AS "직업번호"
         , first_name AS "이름" 
         , last_name AS "성"
         , email AS "이메일"
         , hire_date AS "입사일자"
 ...
 * ● !=, <> : NOT equal 같지 않다
 * ● >, >=, <, <= : 크다, 크거나 같다, 작다, 작거나 같다
 * ● BETWWEN a AND b : 특정값이 a이상, b이하에 포함되어 있다
 ...
 * ● a OR b : a와 b 둘 중 하나라도 조건을 모두 만족해야 True 
 * ● NOT a : a가 모두 아닌 조건. 반대조건
 **/
SELECT *
  FROM employees
...
SELECT *
  FROM employees
 WHERE job_id LIKE '%MAN';  -- 앞쪽 문자열은 뭐든지 상관없고 MAN으로 끝나는 문자를 가진 패턴 찾아라
...
SELECT department_id, dapartment_name
  FROM departments 
